...
def make_withdraw(balance, password):
    ...
    attempts = []
    def bank(amount, pas):
        nonlocal balance, password, count
        if count < 3:
            if pas == password:
                if amount > balance:
                    return 'Insufficient funds'
...
def repeated(t, k):
    ...
    assert k > 1
    "*** YOUR CODE HERE ***"
    # I failed to achieve the second requirement.
    # But I pass the test for the second requirement didn't occur in the test cases.
    last = next(t)
    occur_times = 1
    for x in t:
        if x == last:
